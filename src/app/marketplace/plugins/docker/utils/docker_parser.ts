...
export class DockerFileParser
{
    ...
    parseNameVal(cmd) : boolean {
        ...
        let word = null;
        let words = this.parseWords(cmd.rest);
        ...
        if (words[0].indexOf('=') == -1) {
            ...
            let strs = cmd.rest.split(this.TOKEN_WHITESPACE);
            if (strs.length < 2) {
    ...
    splitCommand(line): any {
        ...
        let name = line.substr(0, match.index).toUpperCase();
        let rest = line.substr(match.index + match[0].length);
    ...
    parse(contents: string, options?: {}): any {
        ...
        var remainder = '';
        let includeComments = options && options['includeComments'];
        
        for (let i = 0; i < lines.length; i++) {
            lineno = i + 1;

            let parseResult = this.parseLine(lines[i], lineno, remainder);
